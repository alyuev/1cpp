...
LINK OK → build_icl\1CPP.dll (~1.95 МБ, 2 043 904 б)**. Проверка `dumpbin`:
...
### Самодостаточность (без libmmd.dll)
Рантайм Intel линкуем статически: `libmmt.lib` (статическая math) + `svml_disp.lib`
(статический SVML) вместо import-либ `libmmd.lib`/`svml_dispmd.lib`. Тогда `1cpp.dll`
не зависит от `libmmd.dll` — только MFC42/MSVCRT/рантайм 1С, как оригинал.

### ✅ ФИНАЛ: работает в 1С (ключевой фикс DllMain)
После линковки был баг «1С не запускается с компонентой» — AV в `frame.dll` при
`ЗагрузитьВнешнююКомпоненту`. Причина: `msvcrt.lib(dllmain.obj)` подсовывал ПУСТОЙ
`_DllMain@12` вместо настоящего из `mfcs42.lib(dllmodul.obj)` (предупреждение
LNK4006), из-за чего MFC не инициализировалась. Фикс: **`mfcs42.lib`+`mfc42.lib`
ПЕРЕД `msvcrt.lib`**. Проверка — тест-база `TestBase\` (`run_base.ps1`):
`ЗагрузитьВнешнююКомпоненту=1`, создаются `HashMap`/`MetaInfoClasses`/
`ИндексированнаяТаблица`. Standalone `LoadLibrary` вне 1cv7 не показатель — так
падает и оригинал (зависимость `Basic.dll` инициализируется только в хосте 1С).
...
forwarder**. Рабочая внешняя компонента `1cpp.dll` получена и **загружается в 1С**.
